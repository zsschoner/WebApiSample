    ...
    public class RestSharpClient
    {
        ...
        public static void Delete(string format)
        {
            ...
            if (id != Guid.Empty)
            {
                ...
                c.DeleteAsync<UserResource>(CreateRequest(Method.DELETE, format),
                                            (resp, result) => WriteResult("DELETE", resp));
            }
        }
        ...
        public static void Put(string format)
        {
            ...
            if (id != Guid.Empty)
            {
                ...
                c.PutAsync<UserModel>(CreateRequest(Method.PUT, "json").AddBody(model),
                                      (resp, result) => WriteResult("PUT", resp));
            }
        }
            ...
            var result = new RestRequest
                             {
                                 Method = method,
                                 JsonSerializer =
                                     new JsonSerializer {ContentType = "application/json; charset=utf-8"},
                                 XmlSerializer =
                                     new XmlSerializer {ContentType = "application/xml; charset=utf-8"}
                             };
        ...
        private static void WriteResult(String method, IRestResponse resp)
        {
            Console.WriteLine(String.Format("==== {0} Result =====",
                                            resp.Request != null ? resp.Request.Method.ToString() : method));
